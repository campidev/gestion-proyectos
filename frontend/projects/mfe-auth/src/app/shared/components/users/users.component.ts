...
export class UsersComponent implements OnInit {
  displayedColumns: string[] = ['username', 'email', 'name', 'lastName', 'status', 'roles', 'actions'];
    ...
    dialogRef.afterClosed().subscribe((result: { assigned: number[], removed: number[] } | undefined) => {
      if (result) {
        ...
        result.assigned.forEach(roleId => {
          this.roleService.assignRole(user.username, roleId).subscribe({next:(value) => this.loadUsers()});
        });
        // Quitar roles seleccionados
        result.removed.forEach(roleId => {
          this.roleService.removeRole(user.username, roleId).subscribe({next:(value) => this.loadUsers()});
        });
